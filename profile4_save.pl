...
{ my $SQLStr=  "SELECT count(*) CNT
  ...
  my $teacherInfo = $cursor->fetchrow_hashref;

  if($teacherInfo->{CNT} > 0) {
	$SQLStr="UPDATE TEACHER SET DEGREE=$P1, TITLE=$P2, POSITION=$P3 WHERE USERID=$P0 ";
	}
  else  { $SQLStr="INSERT INTO TEACHER(USERID, DEGREE, TITLE, POSITION) VALUES($P0, $P1, $P2, $P3) ";
	}
